    ...
    pub(crate) fn dependency_failed(
        ...
        while let Some(key) = pending.pop() {
            ...
            for dep in self
                ...
                .flatten()
            {
                if !self.config.periods.allows(&dep.period, "", at) {
                    continue;
                }
                let Some(master) = state.objects.get(&dep.master) else {
                    return true;
                };
                let code = if master.last_check == 0 {
                    4
                } else if self.config.dependencies.soft_states {
                    numeric(master.status.state)
                } else {
                    master.hard_state
                };
                let criteria = if notification {
                    &dep.notification
                } else {
                    &dep.execution
                };
        ...
        !parents.is_empty()
            && parents.iter().all(|parent| {
                state
                    .objects
                    .get(&host_key(parent))
                    .is_some_and(|r| r.last_check > 0 && numeric(r.status.state) != 0)
            })
    }
