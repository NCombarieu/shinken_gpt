...
impl Engine {
    pub(crate) fn queue_handler(&self, state: &mut Snapshot, key: &str, soft_recovery: bool) {
        ...
        if !state
            ...
            || !r.event_handler_enabled.unwrap_or(
                d.attributes
        ...
        let global = if d.service.is_none() {
            &self.config.global_host_event_handler
        } else {
            &self.config.global_service_event_handler
        };
        let local = d.attributes.get("event_handler").map_or("", String::as_str);
        let commands: Vec<_> = [global.as_str(), local]
            .into_iter()
            .filter(|s| !s.is_empty())
            .map(str::to_owned)
            .collect();
    ...
    async fn run_handler(&self, event: Event) -> String {
        ...
        for command in &event.commands {
            let result = execute::run_with(
                ...
                command,
                &event.macros,
                self.config.event_handler_timeout_ms,
            )
            .await;
            if result.code != 0 {
                eprintln!(
                    "event handler {command} failed for {}: {}",
