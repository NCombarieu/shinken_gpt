...
impl PluginResult {
    pub(crate) fn from_text(code: u8, text: &str, elapsed: f64) -> Self {
        ...
        let mut perf = vec![first_perf.trim().to_owned()];
        for line in lines {
            let (message, data) = line.split_once('|').unwrap_or((line, ""));
            if !message.is_empty() {
                long.push(message.trim_end().to_owned());
            }
            if !data.trim().is_empty() {
                perf.push(data.trim().to_owned());
            }
        }
        Self {
            code,
            output: if output.is_empty() {
                "(no plugin output)".into()
            } else {
        ...
        if let Some(end) = rest.find('$') {
                    ...
                    .all(|c| c.is_ascii_uppercase() || c.is_ascii_digit() || c == '_')
            {
                ...
                result.push_str(
                    macros
                        .get(&key)
                        .ok_or_else(|| format!("unresolved macro {key}"))?,
                );
                rest = &rest[end + 1..];
                continue;
            }
...
fn render(
    ...
    let command = config
        .commands
        .get(&parts[0])
        .ok_or_else(|| format!("unknown command {}", parts[0]))?;
    let host = &config.hosts[&d.host];
    let mut macros = config.resource_macros.clone();
    ...
    macros.insert(
        "$SERVICEDESC$".into(),
        d.service.clone().unwrap_or_default(),
    );
    macros.insert("$TIMET$".into(), (crate::now_ms() / 1000).to_string());
    for (index, arg) in parts.iter().skip(1).enumerate() {
        macros.insert(format!("$ARG{}$", index + 1), arg.clone());
    }
    for (prefix, a) in [("HOST", &host.attributes), ("SERVICE", &d.attributes)] {
        for (key, value) in a {
            if let Some(name) = key.strip_prefix('_') {
                macros.insert(
                    format!("$_{prefix}{}$", name.to_ascii_uppercase()),
                    value.clone(),
                );
            }
        }
    ...
    for _ in 0..32 {
        if !seen.insert(line.clone()) {
            return Err("recursive command macros".into());
        }
...
pub(crate) async fn run_with(
    ...
    let Some(pid) = child.id().and_then(|id| i32::try_from(id).ok()) else {
        ...
        return PluginResult::failure("cannot obtain check process id".into(), 0.0);
    };
    ...
    let result = time::timeout(Duration::from_millis(timeout_ms), async {
        tokio::try_join!(
            ...
            drain(stderr, config.max_output_bytes)
        )
    })
    .await;
    drop(group);
    ...
    match result {
        ...
        Ok(Err(e)) => {
            let _ = child.kill().await;
            let _ = child.wait().await;
            PluginResult::failure(format!("check I/O: {e}"), elapsed)
        }
        Ok(Ok((status, (out, truncated), (err, err_truncated)))) => {
            let code = match status.code() {
                Some(0..=3) => status.code().unwrap_or(3) as u8,
                _ => 3,
            };
            let mut result = PluginResult::from_text(code, &String::from_utf8_lossy(&out), elapsed);
            if !err.is_empty() {
                if !result.long_output.is_empty() {
                    result.long_output.push('\n');
                }
                result.long_output.push_str("stderr: ");
                result
                    .long_output
                    .push_str(String::from_utf8_lossy(&err).trim_end());
            }
            if truncated || err_truncated {
                result.long_output.push_str("\n[plugin output truncated]");
            }
            if status.code().is_none_or(|c| !(0..=3).contains(&c)) {
                result.output = format!(
                    "UNKNOWN: invalid plugin exit status {status}: {}",
                    result.output
                );
            }
            result
...
mod tests {
    ...
    fn plugin_output_and_escaped_arguments() {
        ...
        assert_eq!(r.output, "CRITICAL");
        assert_eq!(r.long_output, "more information\nlast");
