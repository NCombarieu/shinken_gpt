...
use crate::{
    flag, list, positive, required, semantic, value, Attributes, CommandConfig, LoadError,
    TimePeriods,
};
...
fn options(a: &Attributes, key: &str, host: bool) -> Result<BTreeSet<char>, LoadError> {
    ...
    let options: BTreeSet<_> = list(raw)
        .map(|s| {
            ...
            let c = chars
                .next()
                .ok_or_else(|| semantic("empty notification option"))?;
            if chars.next().is_some() || !(if host { "durnfs" } else { "wcurnfs" }).contains(c) {
                return Err(semantic(format!("invalid {key}: {s}")));
            }
            Ok(c)
        })
        .collect::<Result<_, LoadError>>()?;
    Ok(if options.contains(&'n') {
        BTreeSet::new()
    } else {
        options
    })
...
impl NotificationConfig {
    pub(crate) fn build(
        ...
        if enabled {
            periods.validate_use(&period, value(a, "use_timezone", ""))?;
        }
        Ok(Self {
            enabled,
            period,
            options: options(a, "notification_options", host)?,
            interval_ms: (positive(
                value(a, "notification_interval", "60"),
                "notification_interval",
                true,
            )? * interval
                * 1000.0)
                .round() as u64,
            first_delay_ms: (positive(
                value(a, "first_notification_delay", "0"),
...
pub(crate) fn routes(
    ...
    contacts: &BTreeMap<String, Attributes>,
    ...
    for (_, a) in resolved
        .iter()
        .filter(|(kind, _)| *kind == "notificationway")
    {
        let name = required(a, "notificationway_name")?;
        if ways.insert(name, a).is_some() {
            return Err(semantic(format!("duplicate notificationway {name}")));
        }
    }
    ...
    for (name, contact) in contacts {
        ...
        for way in list(value(contact, "notificationways", "")) {
            channels.push((
                ...
                *ways
                    .get(way)
                    .ok_or_else(|| semantic(format!("unknown notificationway {way}")))?,
            ));
        }
        ...
        for (host, kind) in [(true, "host"), (false, "service")] {
            ...
            for (channel, a) in &channels {
                ...
                let contact_options =
                    options(contact, &format!("{kind}_notification_options"), host)?;
                let selected = selected
                    .intersection(&contact_options)
                    .copied()
                    .collect::<BTreeSet<_>>();
                ...
                for command in list(value(a, &format!("{kind}_notification_commands"), "")) {
                    if !commands.contains_key(command.split('!').next().unwrap_or("")) {
                        return Err(semantic(format!(
                            "contact {name} references unknown notification command {command}"
                        )));
                    }
                    periods.validate_use(period, "")?;
                    let route = NotificationRoute {
                        id: format!("{name}/{channel}/{command}"),
                    ...
                    if host {
                        output.host.push(route);
                    } else {
                        output.service.push(route);
                    }
                }
            }
        }
