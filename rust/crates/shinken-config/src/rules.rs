...
use crate::{
    flag, list, positive, required, semantic, value, Attributes, LoadError, MonitoringConfig,
};
...
pub fn service_key(host: &str, service: &str) -> String {
...
pub(crate) fn acyclic(graph: &BTreeMap<String, Vec<String>>, label: &str) -> Result<(), LoadError> {
    ...
    if count != indegree.len() {
        return Err(semantic(format!("{label} cycle")));
    }
    Ok(())
}
pub(crate) fn select_hosts(
    ...
    for token in list(value(a, &format!("{prefix}host_name"), "")) {
        let (negative, name) = token
            .strip_prefix('!')
            .map_or((false, token), |n| (true, n));
        ...
        if name == "*" {
            target.extend(m.hosts.keys().cloned());
        } else if m.hosts.contains_key(name) {
            target.insert(name.to_owned());
        } else {
            return Err(semantic(format!("unknown host {name}")));
        }
    }
    for token in list(value(a, &format!("{prefix}hostgroup_name"), "")) {
        ...
        if name == "*" {
            target.extend(m.hostgroups.values().flatten().cloned());
        } else {
            target.extend(
                m.hostgroups
                    .get(name)
...
fn select_objects(
    ...
    if !service {
        return Ok(hosts.iter().map(|h| host_key(h)).collect());
    }
    let mut selected = BTreeSet::new();
    let mut excluded = BTreeSet::new();
    for token in list(value(a, &format!("{prefix}service_description"), "")) {
        ...
        for host in &hosts {
            let matches: Vec<_> = m
                .services
                .iter()
                .filter(|s| &s.host_name == host && (name == "*" || s.description == name))
                .collect();
            if matches.is_empty() && name != "*" {
                return Err(semantic(format!("unknown service {host}/{name}")));
            }
        ...
        if name == "*" {
            target.extend(
                m.servicegroups
                    .values()
                    .flatten()
                    .map(|(h, s)| service_key(h, s)),
            );
        } else {
            target.extend(
                m.servicegroups
                    .get(name)
                    .ok_or_else(|| semantic(format!("unknown servicegroup {name}")))?
                    .iter()
                    .map(|(h, s)| service_key(h, s)),
            );
        }
...
pub(crate) fn dependencies(
    resolved: &[(&str, Attributes)],
    settings: &Attributes,
    m: &MonitoringConfig,
) -> Result<Dependencies, LoadError> {
    ...
    for (name, host) in &m.hosts {
        let parents: Vec<_> = list(value(&host.attributes, "parents", ""))
            .map(str::to_owned)
            .collect();
        for parent in &parents {
            if !m.hosts.contains_key(parent) {
                return Err(semantic(format!("host {name} has unknown parent {parent}")));
            }
        }
    ...
    for (kind, a) in resolved {
        if !["hostdependency", "servicedependency"].contains(kind) {
            continue;
        }
        ...
        if service
            && !a
                .keys()
                .any(|k| k.starts_with("dependent_host") || k == "dependent_servicegroup_name")
        {
            for name in ["host_name", "hostgroup_name"] {
                if let Some(v) = a.get(name) {
                    child_attrs.insert(format!("dependent_{name}"), v.clone());
                }
            }
        }
        let children = select_objects(&child_attrs, "dependent_", service, m)?;
        if masters.is_empty() || children.is_empty() {
            return Err(semantic(format!("{kind} has an empty object selection")));
        }
        ...
        for child in &children {
            for master in &masters {
                if same_host && child.split('\0').next() != master.split('\0').next() {
                    continue;
                }
                total += 1;
                if total > 100_000 {
                    return Err(semantic("dependency expansion exceeds 100000 edges"));
                }
                output
                    .edges
                    .entry(child.clone())
...
fn escalation(a: &Attributes, host: bool, m: &MonitoringConfig) -> Result<Escalation, LoadError> {
    ...
    let first_ms = if a.contains_key("first_notification_time") {
        Some(milliseconds("first_notification_time")?)
    } else {
        None
    };
    let last_ms = milliseconds("last_notification_time")?;
    if (first_ms.is_none() && last != 0 && last < first)
        || first_ms.is_some_and(|n| last_ms != 0 && last_ms < n)
    {
        return Err(semantic("escalation ends before it starts"));
    }
    if a.contains_key("last_notification_time") && first_ms.is_none() {
        return Err(semantic(
            "last_notification_time needs first_notification_time",
        ));
    }
    ...
    for group in list(value(a, "contact_groups", "")) {
        contacts.extend(
            m.contactgroups
                .get(group)
                .ok_or_else(|| semantic(format!("unknown contactgroup {group}")))?
                .iter()
                .cloned(),
        );
    }
    for name in &contacts {
        if !m.contacts.contains_key(name) {
            return Err(semantic(format!("unknown escalation contact {name}")));
        }
    }
    if contacts.is_empty() {
        return Err(semantic("escalation requires contacts or contact_groups"));
    }
    ...
    Ok(Escalation {
        ...
        interval_ms: a
            ...
            .map(|_| milliseconds("notification_interval"))
            .transpose()?,
...
pub(crate) fn escalations(
    resolved: &[(&str, Attributes)],
    m: &MonitoringConfig,
) -> Result<BTreeMap<String, Vec<Escalation>>, LoadError> {
    let mut result = BTreeMap::<String, Vec<Escalation>>::new();
    let mut named = BTreeMap::new();
    for (kind, a) in resolved {
        if *kind == "escalation" {
            let name = required(a, "escalation_name")?;
            if named.insert(name, a).is_some() {
                return Err(semantic(format!("duplicate escalation {name}")));
            }
        }
        if ["hostescalation", "serviceescalation"].contains(kind) {
            ...
            if selected.is_empty() {
                return Err(semantic(format!("{kind} selects no objects")));
            }
    ...
    for (key, host, a) in m
        .hosts
        .values()
        .map(|h| (host_key(&h.name), true, &h.attributes))
        .chain(m.services.iter().map(|s| {
            (
        ...
        }))
    {
        for name in list(value(a, "escalations", "")) {
            let attrs = named
                .get(name)
                .ok_or_else(|| semantic(format!("unknown escalation {name}")))?;
            result
                .entry(key.clone())
                .or_default()
                .push(escalation(attrs, host, m)?);
        }
    }
    Ok(result)
}
pub(crate) fn validate_handlers(
    ...
    let check = |command: &str| -> Result<(), LoadError> {
        if !command.is_empty()
            && !m
                .commands
                .contains_key(command.split('!').next().unwrap_or(""))
        {
            return Err(semantic(format!("unknown event handler command {command}")));
        }
        Ok(())
    };
    for key in ["global_host_event_handler", "global_service_event_handler"] {
        check(value(settings, key, ""))?;
    }
    for (_, a) in resolved
        .iter()
        .filter(|(kind, _)| ["host", "service"].contains(kind))
    {
...
mod tests {
    ...
    fn compile(input: &str) -> Result<crate::MonitoringConfig, crate::LoadError> {
        build_monitoring_config(&LoadedConfig {
            objects: parse_objects("rules.cfg", input).unwrap(),
    ...
    fn graph_errors_and_criteria_are_rejected() {
        ...
        let edge = |master, child, criteria| {
            format!(
        ...
        assert!(compile(&format!("{valid}{}", edge("b", "a", "d")))
            .unwrap_err()
            .to_string()
            .contains("cycle"));
