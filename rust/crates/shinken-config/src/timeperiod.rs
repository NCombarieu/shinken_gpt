    ...
    for (key, v) in a {
        if let Some(day) = DAYS.iter().position(|name| *name == key) {
            for range in list(v) {
                ...
                if start > end {
                    return Err(format!(
                        "overnight range {range}: split it across two weekdays"
                    ));
                }
                if start != end {
                    period.week[day].push((start, end));
                }
            }
        } else if ![
            "name",
...
impl TimePeriods {
    ...
    fn allows_at(&self, name: &str, day: usize, minute: u32, depth: usize) -> bool {
        ...
        period.week[day]
            ...
            && !period
                .exclude
                .iter()
                .any(|name| self.allows_at(name, day, minute, depth + 1))
    }
    ...
    pub fn next_opening(&self, name: &str, zone: &str, timestamp: u64) -> Option<u64> {
        ...
        if self
            .definitions
            .get(name)
            .is_some_and(|p| p.as_ref().is_ok_and(|p| p.week.iter().all(Vec::is_empty)))
        {
            return None;
        }
