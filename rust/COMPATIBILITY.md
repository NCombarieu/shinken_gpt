...
| --- | --- | --- |
| Configuration tree | Recursive cfg_dir, cfg_file, resource_file, deterministic discovery, symlink-cycle protection | Validated atomic reload via SIGHUP or RELOAD_CONFIG; CLI endpoints require restart |
...
| Check periods | Weekly ranges, fixed `YYYY-MM-DD` exceptions, annual month-date exceptions such as `february 10` and `february -1`, annual month-weekday exceptions such as `monday 1 january` and `friday -1 november`, exclusions, empty periods, IANA timezones and DST | Calendar date ranges, skip intervals, month-relative day ranges and multi-endpoint week-relative exception forms are rejected when referenced |
...
| Dependencies | Host/service failure criteria, pending state, periods, inherited dependencies, graph validation, SOFT-state option | Cross-object check ordering is eventual; parent changes trigger confirmation checks |
| Event handlers | Global then local, SOFT retries, HARD changes/recovery, macros, deadlines, runtime toggles | Bounded volatile queue; no exactly-once effects or strict notification ordering |
| Escalations | Host/service/group targets, named Shinken rules, numeric/time ranges, contact union, shortest interval | Recovery intentionally reaches every successful recipient of the incident |
...
| Notifications | Native host/service commands, contact groups, notificationways, state/period filters, first delay, intervals, acknowledgements/downtime suppression, recovery and retained delivery history | Escalations and native event handlers are supported; acknowledgement/flapping/downtime messages and durable queues are absent |
