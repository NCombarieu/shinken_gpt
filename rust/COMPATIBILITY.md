...
| Check periods | Weekly ranges, fixed `YYYY-MM-DD` exceptions, annual month-date exceptions such as `february 10` and `february -1`, exclusions, empty periods, IANA timezones and DST | Calendar date ranges, skip intervals, month-relative day ranges and week-relative exception forms are rejected when referenced |
