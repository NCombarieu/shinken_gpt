...
class LiveStatusRegenerator(Regenerator):
    ...
    def all_done_linking(self, inst_id):
        ...
        [self.hosts._id_contact_heap.setdefault(get_obj_full_name(c), []).append(k) for (k, v) in self.hosts.items.items() for c in v.contacts]
        for c in list(self.hosts._id_contact_heap.keys()):
        ...
        if self.service_authorization_strict:
            [self.services._id_contact_heap.setdefault(get_obj_full_name(c), []).append(k) for (k, v) in self.services.items.items() for c in v.contacts]
        else:
            # 1. every host contact automatically becomes a service contact
            [self.services._id_contact_heap.setdefault(get_obj_full_name(c), []).append(k) for (k, v) in self.services.items.items() for c in v.host.contacts]
            # 2. explicit service contacts
            [self.services._id_contact_heap.setdefault(get_obj_full_name(c), []).append(k) for (k, v) in self.services.items.items() for c in v.contacts]
        # services without contacts inherit the host's contacts (no matter of strict or loose)
        [self.services._id_contact_heap.setdefault(get_obj_full_name(c), []).append(k) for (k, v) in self.services.items.items() if not v.contacts for c in v.host.contacts]
        for c in list(self.services._id_contact_heap.keys()):
            # remove duplicates
        ...
        if self.group_authorization_strict:
            for c in list(self.hosts._id_contact_heap.keys()):
                ...
                for (k, v) in self.hostgroups.items.items():
            ...
            for c in list(self.services._id_contact_heap.keys()):
                ...
                for (k, v) in self.servicegroups.items.items():
                    # now look if c is contact of all v.members
            ...
            [self.hostgroups._id_contact_heap.setdefault(get_obj_full_name(c), []).append(k) for (k, v) in self.hostgroups.items.items() for h in v.members for c in h.contacts]
            [self.servicegroups._id_contact_heap.setdefault(get_obj_full_name(c), []).append(k) for (k, v) in self.servicegroups.items.items() for s in v.members for c in s.contacts] # todo: look at mk-livestatus. what about service's host contacts?
        for c in list(self.hostgroups._id_contact_heap.keys()):
            # remove duplicates
        ...
        for c in list(self.servicegroups._id_contact_heap.keys()):
            # remove duplicates
        ...
        setattr(self.hosts, '_id_by_host_name_heap', dict([(get_obj_full_name(v), k) for (k, v) in self.hosts.items.items()]))
        setattr(self.services, '_id_by_service_name_heap', dict([(get_obj_full_name(v), k) for (k, v) in self.services.items.items()]))
        setattr(self.services, '_id_by_host_name_heap', dict())
        [self.services._id_by_host_name_heap.setdefault(get_obj_full_name(v.host), []).append(k) for (k, v) in self.services.items.items()]
        logger.debug("[Livestatus Regenerator] Id by Hostname heap: %s" % str(self.services._id_by_host_name_heap))
        for hn in list(self.services._id_by_host_name_heap.keys()):
        ...
        setattr(self.hostgroups, '_id_by_hostgroup_name_heap', dict([(get_obj_full_name(v), k) for (k, v) in self.hostgroups.items.items()]))
        setattr(self.servicegroups, '_id_by_servicegroup_name_heap', dict([(get_obj_full_name(v), k) for (k, v) in self.servicegroups.items.items()]))
        ...
        [self.hosts._id_by_hostgroup_name_heap.setdefault(get_obj_full_name(hg), []).append(k) for (k, v) in self.hosts.items.items() for hg in v.hostgroups]
        for hg in list(self.hosts._id_by_hostgroup_name_heap.keys()):
        ...
        [self.services._id_by_servicegroup_name_heap.setdefault(get_obj_full_name(sg), []).append(k) for (k, v) in self.services.items.items() for sg in v.servicegroups]
        for sg in list(self.services._id_by_servicegroup_name_heap.keys()):
        ...
        [self.services._id_by_hostgroup_name_heap.setdefault(get_obj_full_name(hg), []).append(k) for (k, v) in self.services.items.items() for hg in v.host.hostgroups]
        for hg in list(self.services._id_by_hostgroup_name_heap.keys()):
