...
class LiveStatusQuery(object):
    ...
    def split_option(self, line, splits=1):
        ...
        x = [(i.isdigit() and int(i)) or i for i in [token.strip() for token in re.split(r"[\s]+", line, splits)]]
        return x
    ...
    def parse_input(self, data):
        ...
        for line in data.splitlines():
            ...
            if keyword == 'GET':  # Get the name of the base table
                ...
                if self.table not in list(table_class_map.keys()):
                ...
                separators = [chr(int(sep)) for sep in line.split(' ', 5)[1:]]
    ...
    def launch_query(self):
        ...
        try:
            ...
            if self.stats_query:
                self.columns = list(range(num_stats_filters))
    ...
    def get_status_livedata(self, cs):
        return [c for c in list(self.datamgr.rg.configs.values())]
    ...
    def get_columns_livedata(self, cs):
        ...
        for table in tablenames:
            ...
            for attribute in cls.lsm_columns:
                result.append({
                    'description': getattr(cls, 'lsm_' + attribute).description,
                    'name': attribute,
                    'table': table,
                    'type': {
                        int: 'int',
                        ...
                        list: 'list',
                        str: 'string',
                    }[getattr(cls, 'lsm_' + attribute).datatype],
                })
        ...
        for column in self.outputcolumns:
            try:
                ...
                if hasattr(item, 'lsm_'+column):
                    ...
                    value = getattr(item, 'lsm_'+column).default
                else:
    ...
    def statsify_result(self, filtresult):
        ...
        if self.stats_group_by:
            ...
            for group in groupedresult:
                ...
                resultdict[group] = dict(list(zip(self.stats_group_by, group)))
        ...
        for i in range(maxidx):
            ...
            if self.stats_group_by:
                ...
                for group in groupedresult:
                    resultdict[group][stats_number] = postprocess(list(filter(filtfunc, groupedresult[group])))
            else:
                # Calc statistics over _all_ elements of filtresult
                if isinstance(filtresult, list):
                    resultdict[stats_number] = postprocess(list(filter(filtfunc, filtresult)))
                else:
                    # it's a generator
    ...
    def make_filter(self, operator, attribute, reference):
        if reference is not None:
            ...
            # Python 3 str is already text; no bytes->unicode decode needed.
        ...
        def eq_filter(item):
            try:
                ...
                if hasattr(item, attribute):
                    return getattr(item.__class__, attribute).default == reference
                else:
        ...
        def ge_contains_filter(item):
            try:
                if getattr(item, attribute).datatype == list:
        ...
        if operator == '=':
            ...
            raise Exception("wrong operation: %s" % operator)
