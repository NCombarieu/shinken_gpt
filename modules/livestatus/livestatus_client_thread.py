...
class LiveStatusClientThread(threading.Thread):
    ...
    def _send_data(self, data):
        if not data:
            return
        # Response building stays in str (like the rest of this Python
        # 2-era pipeline); encode to bytes only right before the socket
        # write, which is all Python 3 sockets accept.
    ...
    def run(self):
        ...
        try:
            while not self.stop_requested:
                ...
                # The rest of the livestatus query pipeline was written for
                # Python 2, where str was bytes: decode once here so string
                # ops like "':' in line" downstream keep working on Python 3.
                self.handle_request(request_bytes.decode('utf-8', 'replace'))
